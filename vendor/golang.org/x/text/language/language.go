...
// private reports whather the Tag consists solely of a private use tag.
...
// The confidence level indicates whather a value was explicitly specified,
// whather it is typically the only possible value, or whather there is
// an ambiguity.
type Confidence int
...
// wasn't found. The hasExt return value reports whather an -u extension was present.
...
func (t Tag) findTypeForKey(key string) (start, end int, hasExt bool) {
	...
	for {
		...
		if p3 := p + 3; s[p3] == '-' {
			// Found a key.
			// Check whather we just processed the key that was requested.
			if curKey == key {
				return start, p, true
			}
...
// IsCountry returns whather this region is a country or autonomous area. This
...
// IsGroup returns whather this region defines a collection of regions. This
// includes non-standard definitions from CLDR.
...
// Contains returns whather Region c is contained by Region r. It returns true
// if c == r.
