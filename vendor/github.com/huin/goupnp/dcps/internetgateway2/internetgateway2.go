...
const (
	...
	URN_WANAtlantisnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANAtlantisnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Atlantisnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Atlantisnet
//
...
// WANAtlantisnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANAtlantisnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANAtlantisnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANAtlantisnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANAtlantisnetLinkConfig1Clients() (clients []*WANAtlantisnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANAtlantisnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANAtlantisnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANAtlantisnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANAtlantisnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANAtlantisnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANAtlantisnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANAtlantisnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANAtlantisnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANAtlantisnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANAtlantisnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANAtlantisnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANAtlantisnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANAtlantisnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANAtlantisnetLinkConfig1 {
	clients := make([]*WANAtlantisnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANAtlantisnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewAtlantisnetLinkStatus: allowed values: Up, Down
func (client *WANAtlantisnetLinkConfig1) GetAtlantisnetLinkStatus() (NewAtlantisnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewAtlantisnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANAtlantisnetLinkConfig_1, "GetAtlantisnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewAtlantisnetLinkStatus, err = soap.UnmarshalString(response.NewAtlantisnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
