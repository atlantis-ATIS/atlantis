...
func NewLesServer(ath *ath.Atlantis, config *ath.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(ath.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, ath.EventMux(), ath.Engine(), newPeerSet(), ath.BlockChain(), ath.TxPool(), ath.ChainDb(), nil, nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(ath.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(ath.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(ath.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(ath.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(ath.ChainDb())
	return srv, nil
}
...
func newCostStats(db athdb.Database) *requestCostStats {
