...
import (
	...
	"github.com/athereum/go-athereum/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gath console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = ath.getBlock(ath.getTransaction(tx).blockHash);
  var genesis = ath.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ath.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ath.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func TestCallTracer(t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(strings.TrimPrefix(file.Name(), "call_tracer_"), ".json")), func(t *testing.T) {
			...
			statedb := tests.MakePreState(athdb.NewMemDatabase(), test.Genesis.Alloc)
