...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Atlantis
...
func (ath *Atlantis) startBloomHandlers() {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-ath.shutdownChan:
					return

				case request := <-ath.bloomRequests:
					task := <-request
...
// for the Atlantis header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  athdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db athdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := athdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
