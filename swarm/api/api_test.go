...
import (
	...
	"github.com/athereum/go-athereum/swarm/storage"
)
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.ath"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.ath"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.ath"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	athAddr := "swarm.ath"
	athHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	athResolve := newTestResolver(athHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(athResolve, "")),
			addr:   athAddr,
			result: athHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(athResolve, ""),
				MultiResolverOptionWithResolver(athResolve, ""),
			),
			addr:   athAddr,
			result: athHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(athResolve, "ath"),
			),
			addr:   athAddr,
			result: athHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "ath"),
				MultiResolverOptionWithResolver(doesntResolve, "ath"),
				MultiResolverOptionWithResolver(athResolve, "ath"),
			),
			addr:   athAddr,
		...
		},
		{
			...
			addr:   athAddr,
			result: athHash,
			err:    errors.New(`DNS name not found: "swarm.ath"`),
		},
		{
