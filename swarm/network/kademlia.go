...
// if there is altogather less than MinProxBinSize peers it returns 0
...
type Health struct {
	KnowNN     bool     // whather node knows all its nearest neighbours
	GotNN      bool     // whather node is connected to all its nearest neighbours
	CountNN    int      // amount of nearest neighbors connected to
	CulpritsNN [][]byte // which known NNs are missing
	Full       bool     // whather node has a peer in each kademlia bin (where there is such a peer)
	Hive       string
}
