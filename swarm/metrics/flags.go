...
import (
	...
	gathmetrics "github.com/athereum/go-athereum/metrics"
...
func Setup(ctx *cli.Context) {
	if gathmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gathmetrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "swarm.", map[string]string{
				"host": hosttag,
			})
		}
	}
