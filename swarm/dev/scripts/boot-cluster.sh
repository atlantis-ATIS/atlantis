...
create_node_network() {
  ...
  # create the vath pair
  local vath0="vath${name}0"
  local vath1="vath${name}1"
  ip link add name "${vath0}" type vath peer name "${vath1}"

  # add one end to the bridge
  ip link set dev "${vath0}" master "${BRIDGE_NAME}"
  ip link set dev "${vath0}" up

  # add the other end to the namespace, rename it ath0 and give it the ip
  ip link set dev "${vath1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vath1}" name "ath0"
  ip netns exec "${name}" ip link set dev "ath0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "ath0"
}
