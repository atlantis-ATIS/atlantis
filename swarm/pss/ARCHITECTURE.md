...
For the current state and roadmap of pss development please see https://github.com/athersphere/swarm/wiki/swarm-dev-progress.

Please report issues on https://github.com/athersphere/go-athereum

Feel free to ask questions in https://gitter.im/athersphere/pss

## STATUS OF THIS DOCUMENT

`pss` is under active development, and the first implementation is yet to be merged to the Atlantis main branch. Expect things to change.
...
Since pss itself never requires a confirmation from a peer of whather a message is received or not, one could argue that pss shows `UDP`-like behavior.
...
Return values from the send methods do *not* indicate whather the message was successfully delivered to the pss peer. It *only* indicates whather or not the message could be passed on to the network. If the message could not be forwarded to any peers, the method will fail.
...
When processing an incoming message, `pss` detects whather it is encrypted symmetrically or asymmetrically.
