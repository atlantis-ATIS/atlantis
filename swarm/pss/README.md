...
The code tutorial [p2p programming in go-athereum](https://github.com/nolash/go-athereum-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-athereum` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-athereum` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.
...
returns:
1. whather key was successfully removed (bool)
```
