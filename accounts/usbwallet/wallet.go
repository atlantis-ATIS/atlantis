...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Atlantis
...
type wallet struct {
	...
	deriveChain   atlantis.ChainStateReader // Blockchain state reader to discover used account with
...
// whather they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Atlantis account
			...
			// If the next account is empty, stop self-derivation, but add it nonatheless
...
// Contains implements accounts.Wallet, returning whather a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chainatlantis.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
