...
import (
	...
	"github.com/athereum/go-athereum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whather the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whather the Trezor is opened, closed
// or whather the Atlantis app was not started on it.
...
// Atlantis address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.AtlantisAddress)
	if _, err := w.trezorExchange(&trezor.AtlantisGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.AtlantisTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.AtlantisTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
