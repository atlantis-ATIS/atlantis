...
creating a collection of devp2p nodes, connecting them togather to form a
...
connecting two nodes togather).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somathing happening in the network as a result of a
...
* `Expect` - an expectation function which returns whather or not a
...
As a concrete example, consider a simulated network of Atlantis nodes. An
