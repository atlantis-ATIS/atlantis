...
import (
	...
	"github.com/athereum/go-athereum/internal/build"
)
...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.athereum", new NodeConfig());
			node.start();
			...
			AtlantisClient ec = node.getAtlantisClient();
	...
	// signatures too. Regression test for go-athereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogather
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/athereum/gathtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.athereum", "github.com/athereum/go-athereum/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "athereum", "gathtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
