...
import (
	...
	"github.com/athereum/go-athereum/signer/core"
	"github.com/athereum/go-athereum/signer/storage"
)
...
func (alwaysDenyUI) OnApprovedTx(tx athapi.SignTransactionResult) {
	panic("implement me")
}
...
func (d *dummyUI) OnApprovedTx(tx athapi.SignTransactionResult) {
...
func TestForwarding(t *testing.T) {
	...
	r.OnApprovedTx(athapi.SignTransactionResult{})
...
func TestLimitWindow(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		response := athapi.SignTransactionResult{
			Tx:  dummySigned(v),
...
func (d *dontCallMe) OnApprovedTx(tx athapi.SignTransactionResult) {
	d.t.Fatalf("Did not expect next-handler to be called")
}
