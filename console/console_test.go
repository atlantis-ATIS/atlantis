...
func newTester(t *testing.T, confOverride func(*ath.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Atlantis service within
	...
	athConf := &ath.Config{
		...
		Atlantisbase: common.HexToAddress(testAddress),
		Ethash: athash.Config{
			PowMode: athash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(athConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return ath.New(ctx, athConf) }); err != nil {
		t.Fatalf("failed to register Atlantis protocol: %v", err)
	}
