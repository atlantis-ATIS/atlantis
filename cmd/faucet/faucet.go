...
// faucet is a Atlantis faucet backed by a light client.
package main
...
var (
	...
	athPortFlag = flag.Int("athport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Atlantis protocol")
	statsFlag   = flag.String("athstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Atlantiss to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Atlantis and the faucet")
)

var (
	ather = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Atlantiss", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *athPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Atlantis address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Atlantis light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Atlantis protocol stack
	client *athclient.Client   // Client connection to the Atlantis chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Atlantis light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := ath.DefaultConfig
	...
	// Assemble the athstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
	...
	client := athclient.NewClient(api)
...
// close terminates the Atlantis connection and tears down the faucet.
...
// apiHandler handles requests for Atlantis grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, ather),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the Atlantis address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somathing funky happened, please open an issue at https://github.com/athereum/go-athereum/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), ather)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, ather)
...
// the username, avatar URL and Atlantis address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Atlantis addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Atlantis address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Atlantis address and profile URL.
...
// returning the username, avatar URL and Atlantis address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Atlantis address,
