...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.athstats == "" {
		log.Error("No athstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.athashdir == "" {
			fmt.Printf("Where should the athash mining DAGs be stored on the remote machine?\n")
			infos.athashdir = w.readString()
		} else {
			fmt.Printf("Where should the athash mining DAGs be stored on the remote machine? (default = %s)\n", infos.athashdir)
			infos.athashdir = w.readDefaultString(infos.athashdir)
		}
	}
	...
	if infos.athstats == "" {
		...
		infos.athstats = w.readString() + ":" + w.conf.athstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.athstats)
		infos.athstats = w.readDefaultString(infos.athstats) + ":" + w.conf.athstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an atherbase to mine against
			fmt.Println()
			if infos.atherbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.atherbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.atherbase)
				infos.atherbase = w.readDefaultAddress(common.HexToAddress(infos.atherbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Atlantis node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
