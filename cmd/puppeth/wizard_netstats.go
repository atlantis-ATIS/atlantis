...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for athstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["athstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["athstats"] = infos.Report()
		athstats = infos.config
	}
