...
// nodeDockerfile is the Dockerfile required to run an Atlantis node.
var nodeDockerfile = `
FROMatlantis/client-go:latest
...
RUN \
  echo 'gath --cache 512 init /genesis.json' > gath.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.athereum/keystore/ && cp /signer.json /root/.athereum/keystore/' >> gath.sh && \{{end}}
	echo $'gath --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --athstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Atlantisbase}}--atherbase {{.Atlantisbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gath.sh

ENTRYPOINT ["/bin/sh", "gath.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Atlantis node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.athereum{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.athash{{end}}
    environment:
      - PORT={{.Port}}/tcp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Ethstats}}
      - MINER_NAME={{.Atlantisbase}}
...
// deployNode deploys a new Atlantis node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.atherbase == "" {
		kind = "bootnode"
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Ethstats":  config.athstats,
		"Atlantisbase": config.atherbase,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Ethashdir":  config.athashdir,
		"Network":    network,
		"Port":       config.port,
		"TotalPeers": config.peersTotal,
		"Light":      config.peersLight > 0,
		"LightPeers": config.peersLight,
		"Ethstats":   config.athstats[:strings.Index(config.athstats, ":")],
		"Atlantisbase":  config.atherbase,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":        info.athstats,
	}
	if info.gasTarget > 0 {
		...
		if info.atherbase != "" {
			...
			report["Ethash directory"] = info.athashdir
			report["Miner account"] = info.atherbase
		}
		if info.keyJSON != "" {
...
// whather it's running, and if yes, whather it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gath --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.athereum"],
		athashdir:  infos.volumes["/root/.athash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		athstats:   infos.envvars["STATS_NAME"],
		atherbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
