...
FROM puppath/explorer:latest

ADD athstats.json /athstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../ath-net-intelligence-api && pm2 start /athstats.json)' >  explorer.sh && \
	echo '(cd ../atherchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerEthstats is the configuration file for the athstats javascript client.
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.athereum
    environment:
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	athstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerEthstats)).Execute(athstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.athstats[:strings.Index(config.athstats, ":")],
		"Secret": config.athstats[strings.Index(config.athstats, ":")+1 : strings.Index(config.athstats, "@")],
		"Host":   config.athstats[strings.Index(config.athstats, "@")+1:],
	})
	files[filepath.Join(workdir, "athstats.json")] = athstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.athereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		athstats: infos.envvars["STATS"],
	}
	return stats, nil
}
