...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"athstats", "explorer", "wallet", "faucet"} {
	...
	// If we have athstats running, ask whather to make the secret public or not
	if w.conf.athstats != "" {
		fmt.Println()
		fmt.Println("Include athstats secret on dashboard (y/n)? (default = yes)")
