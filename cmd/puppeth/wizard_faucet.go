...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Atlantiss to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.athstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.athstats = w.readString() + ":" + w.conf.athstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.athstats)
		infos.node.athstats = w.readDefaultString(infos.node.athstats) + ":" + w.conf.athstats
	}
