...
// makeWizard creates and returns a new puppath wizard.
...
// setting up a new or managing an existing Atlantis private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppath, your Atlantis private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Atlantis network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and athstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppath uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Atlantis network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppath", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somathing I can do")
		}
	}
}
