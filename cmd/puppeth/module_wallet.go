...
FROM puppath/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gath --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'gath --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --athstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppathNetworkID/{{.NetworkID}}/g' dist/js/atherwallet-master.js && \
	sed -i 's/PuppathNetwork/{{.Network}}/g'     dist/js/atherwallet-master.js && \
	sed -i 's/PuppathDenom/{{.Denom}}/g'         dist/js/atherwallet-master.js && \
	sed -i 's/PuppathHost/{{.Host}}/g'           dist/js/atherwallet-master.js && \
	sed -i 's/PuppathRPCPort/{{.RPCPort}}/g'     dist/js/atherwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whather
// it's running, and if yes, whather it's responsive.
