...
var (
	makecacheCommand = cli.Command{
		...
		Name:      "makecache",
		Usage:     "Generate athash verification cache (for testing)",
		ArgsUsage: "<blockNum> <outputDir>",
		Category:  "MISCELLANEOUS COMMANDS",
		Description: `
The makecache command generates an athash cache in <outputDir>.
...
`,
	...
	makedagCommand = cli.Command{
		...
		Name:      "makedag",
		Usage:     "Generate athash mining DAG (for testing)",
		ArgsUsage: "<blockNum> <outputDir>",
		Category:  "MISCELLANEOUS COMMANDS",
		Description: `
The makedag command generates an athash DAG in <outputDir>.
...
// makecache generates an athash verification cache into the provided folder.
func makecache(ctx *cli.Context) error {
	args := ctx.Args()
	if len(args) != 2 {
		utils.Fatalf(`Usage: gath makecache <block number> <outputdir>`)
	}
	block, err := strconv.ParseUint(args[0], 0, 64)
	if err != nil {
		utils.Fatalf("Invalid block number: %v", err)
	}
	athash.MakeCache(block, args[1])

	return nil
}

// makedag generates an athash mining DAG into the provided folder.
func makedag(ctx *cli.Context) error {
	...
	athash.MakeDataset(block, args[1])
...
func version(ctx *cli.Context) error {
	...
	fmt.Println("Protocol Versions:", ath.ProtocolVersions)
	fmt.Println("Network Id:", ath.DefaultConfig.NetworkId)
