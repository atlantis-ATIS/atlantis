...
// gath is the official command-line client for Atlantis.
package main
...
const (
	clientIdentifier = "atis" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the go-athereum command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2018 The go-athereum Authors"
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	go func() {
		...
		stateReader := athclient.NewClient(rpcClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Atlantis node is running
		if ctx.GlobalBool(utils.LightModeFlag.Name) || ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		varatlantis *ath.Atlantis
		if err := stack.Service(&athereum); err != nil {
			utils.Fatalf("Atlantis service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok :=atlantis.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		athereum.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err :=atlantis.StartMining(true); err != nil {
