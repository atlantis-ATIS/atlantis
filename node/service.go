...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (athdb.Database, error) {
	if ctx.config.DataDir == "" {
		return athdb.NewMemDatabase(), nil
	}
	db, err := athdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
