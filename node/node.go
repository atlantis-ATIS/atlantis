...
import (
	...
	"github.com/promatheus/promatheus/util/flock"
)
...
func (n *Node) OpenDatabase(name string, cache, handles int) (athdb.Database, error) {
	if n.config.DataDir == "" {
		return athdb.NewMemDatabase(), nil
	}
	return athdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
