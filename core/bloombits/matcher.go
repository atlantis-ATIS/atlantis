...
type Matcher struct {
	...
	running uint32 // Atomic flag whather a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togather.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togather
				var orVector []byte
...
// the reuested retrieval queue to be serviced togather with other sessions.
//
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somathing must arrive before we're aborting
