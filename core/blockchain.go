...
// Package core implements the Atlantis consensus protocol.
package core
...
type BlockChain struct {
	...
	db     athdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default Atlantis Validator and
// Processor.
func NewBlockChain(db athdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= athdb.IdealBatchSize {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - athdb.IdealBatchSize)
			}
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	// Sanity check that we have somathing meaningful to import
	if len(chain) == 0 {
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonatheless.
...
//
// The verify parameter can be used to fine tune whather nonce verification
